...
test('ignores malformed room events without throwing', () => {
  const rooms = new Map()

  assert.doesNotThrow(() => {
    applyRoomEvent(rooms, { type: 'ROOM_JOINED' })
    applyRoomEvent(rooms, { type: 'ROOM_STARTED', payload: null })
    applyRoomEvent(rooms, { type: 'ROOM_CREATED', payload: { roomCode: '' } })
  })

  assert.equal(rooms.size, 0)
})

test('does not create a room when a join event targets an unknown room', () => {
  const rooms = new Map()

  assert.doesNotThrow(() => {
    applyRoomEvent(rooms, {
      type: 'ROOM_JOINED',
      payload: { roomCode: 'NOPE01', challengerName: 'Guest' },
    })
  })

  assert.equal(rooms.has('NOPE01'), false)
})
