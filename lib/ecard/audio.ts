...
class AudioEngine {
  ...
  private clockTick: { interval: number | null } = { interval: null }
  private muted = false
  ...
  startClockTick() {
    const ctx = this.ensure()
    if (!ctx || !this.master || this.muted || this.clockTick.interval) return
    const tick = () => {
    ...
    tick()
    this.clockTick.interval = window.setInterval(tick, 1000)
  }

  stopClockTick() {
    if (this.clockTick.interval) window.clearInterval(this.clockTick.interval)
    this.clockTick.interval = null
  }
  ...
  win() {
    ...
    window.setTimeout(() => this.tone(660, 0.12, 'triangle', 0.14), 70)
  }

  lose() {
    this.tone(150, 0.16, 'sine', 0.18)
    window.setTimeout(() => this.tone(88, 0.24, 'sine', 0.14), 120)
  }
