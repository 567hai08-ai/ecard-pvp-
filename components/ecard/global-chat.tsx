  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-zinc-800 px-3 py-2 bg-[#121212]">
        <h3 className="font-display text-sm uppercase tracking-[0.22em] text-[#f2c96b]">Cổng Chat Thế Giới</h3>
        <span
      ...
      <div ref={listRef} className="thin-scroll flex-1 space-y-1.5 overflow-y-auto px-3 py-2 font-sans text-sm">
        {messages.length === 0 && (
          <p className="mt-4 text-center font-sans text-xs text-zinc-400 italic">Chưa có ai lên tiếng...</p>
        )}
        {messages.map((m) => (
      ...
      <div className="relative border-t border-zinc-800 p-2 bg-[#0a0a0a]">
        ...
        <div className="flex gap-2">
          <input
            ...
            className={`flex-1 rounded border bg-[#050505] px-3 py-1.5 font-sans text-sm text-zinc-100 outline-none transition-all duration-300 ${
          ...
          <button
            type="button"
            onClick={submit}
            className="rounded border border-[#b3914a]/50 bg-[#b3914a]/10 px-4 py-1.5 font-sans text-xs font-bold uppercase tracking-[0.24em] text-[#f2c96b] hover:bg-[#b3914a]/20 transition-colors"
          >
            Gửi
          </button>
        </div>
