...
import { type ChatMessage, type GlobalChatMessage, type SpeechBubble, HYODO_LINES, type SystemBalanceWeights } from '@/components/ecard/types'
...
// Falls back to Railway-deployed server if env var not set.
// the WebSocketNetworkEngine below degrades gracefully when it can't connect.
const WEBSOCKET_URL = process.env.NEXT_PUBLIC_WEBSOCKET_URL || 'wss://ecard-pvp-production.up.railway.app'
...
export function EcardGame() {
  ...
  const requestAiApproval = useCallback((): Promise<{ allowed: boolean; remainingMs?: number }> => {
    return new Promise((resolve) => {
      ...
      if (!engine || !engine.getConnectionStatus()) {
        resolve({ allowed: true })
        return
      }

      let settled = false
      const finish = (result: { allowed: boolean; remainingMs?: number }) => {
        if (settled) return
        settled = true
      ...
      function onRejected(data: any) {
        finish({ allowed: false, remainingMs: data?.remainingMs ?? 0 })
      }
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-6xl flex-col gap-4 px-4 pb-6 pt-16">
        ...
        <div className="flex flex-1 items-center justify-between gap-2">
          ...
          <div className="relative flex flex-col items-center">
            ...
            <Silhouette faction={enemyFaction} side="right" split={split && ending === 'victory'} injuryLevel={Math.max(0, Math.min(1, 1 - enemyHP / START_HP))} label={enemyLabel} />
