...
2. Trên máy, trong thư mục project, chạy:
   ```bash
   git init
   git add .
...
Mở file `.env.local` ở thư mục gốc project, điền vào dòng cuối:

```bash
NEXT_PUBLIC_WEBSOCKET_URL=wss://kaiji-global-hub.onrender.com
```

Lưu file, restart `npm run dev` (Ctrl+C rồi chạy lại `npm run dev`) để app đọc
biến môi trường mới.
